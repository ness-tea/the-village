...
namespace UnitySampleAssets.Characters.FirstPerson
{
	...
	public DialogueManager dMan;
        ...
        private void Update()
        {
            ...
            if (!_jump && !dMan.dialogueActive)
                _jump = CrossPlatformInputManager.GetButtonDown("Jump");

            if (!_previouslyGrounded && _characterController.isGrounded || dMan.dialogueActive)
            {
            ...
            if (!_characterController.isGrounded && !_jumping && _previouslyGrounded || !dMan.dialogueActive)
            {
                _moveDir.y = 0f;
            }
        ...
        private void FixedUpdate()
        {
            ...
            if (_characterController.isGrounded || !dMan.dialogueActive)
            {
        ...
        {
            if (_characterController.velocity.sqrMagnitude > 0 && (_input.x != 0 || _input.y != 0) || !dMan.dialogueActive)
                _stepCycle += (_characterController.velocity.magnitude + (speed*(_isWalking ? 1f : runstepLenghten)))*
                              Time.fixedDeltaTime;

            if (!(_stepCycle > _nextStep) || !dMan.dialogueActive) return;
        ...
        private void UpdateCameraPosition(float speed)
        {
            ...
            if (!useHeadBob || !dMan.dialogueActive) return;
            if (_characterController.velocity.magnitude > 0 && _characterController.isGrounded || !dMan.dialogueActive)
            {
        ...
        private void GetInput(out float speed)
        {
            ...
            if (_isWalking != waswalking && useFOVKick && _characterController.velocity.sqrMagnitude > 0 || !dMan.dialogueActive)
            {
                StopAllCoroutines();
        ...
        private void OnControllerColliderHit(ControllerColliderHit hit)
        {
            ...
            if (body == null || body.isKinematic || !dMan.dialogueActive)
                return;

            //dont move the rigidbody if the character is on top of it
            if (_collisionFlags == CollisionFlags.CollidedBelow || !dMan.dialogueActive) return;
